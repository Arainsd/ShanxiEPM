...
namespace hc.epm.Web.Controllers
{
    ...
    /// 初次谈判
    /// </summary>
    public class TzFirstNegotiationController : BaseWebController
    {
        // GET: TzFirstNegotiation
        /// <summary>
        /// 列表查询
        /// </summary>
        /// <param name="projectName"></param>
        /// <param name="projectNature"></param>
        ...
        /// <returns></returns>
        [AuthCheck(Module = WebCategory.TzFirstNegotiation, Right = SystemRight.Browse)]
        public ActionResult Index(string projectName = "", string projectNature = "", string stationName = "", string companyId = "", string startTime = "", string projectState = "", string endTime = "", int pageIndex = 1, int pageSize = 10)
        {
            ...
            using (ClientSiteClientProxy proxy = new ClientSiteClientProxy(ProxyEx(Request)))
            {
                ...
                result = proxy.GetTzFirstNegotiationList(qc);
        ...
        /// </summary>
        /// <returns></returns>
        [AuthCheck(Module = WebCategory.TzFirstNegotiation, Right = SystemRight.Modify)]
        public ActionResult Add(long projectId)
        {
            using (ClientSiteClientProxy proxy = new ClientSiteClientProxy(ProxyEx(Request)))
            {
                ...
                var result = proxy.GetTzFirstNegotiationModel(projectId);
                
                // 谈判结果
                ViewBag.LandNatureCode = subjects[DictionaryType.NegotiateResult].ToSelectList("Name", "No", false, result.Data.TzFirstNegotiation.TalkResultType ?? "");
        ...
        [HttpPost]
        [AuthCheck(Module = WebCategory.TzFirstNegotiation, Right = SystemRight.Modify)]
        public ActionResult Add(Epm_TzFirstNegotiation model)
        {
            ...
            using (ClientSiteClientProxy proxy = new ClientSiteClientProxy(ProxyEx(Request)))
            {
                result = proxy.AddTzFirstNegotiation(model);
            }
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        [AuthCheck(Module = WebCategory.TzFirstNegotiation, Right = SystemRight.Info)]
        public ActionResult Detail(long id)
        {
            using (ClientSiteClientProxy proxy = new ClientSiteClientProxy(ProxyEx(Request)))
            {
                var result = proxy.GetTzFirstNegotiationModel(id);
                if (result.Flag == EResultFlag.Success && result.Data != null)
                {
                    return View(result.Data);
                }
