...
namespace hc.epm.Admin.Web.Controllers
{
    ...
    {
        // GET: EmailTemplete
        [AuthCheck(Module = AdminModule.EmailTemplete, Right = SystemRight.Browse)]
        public ActionResult Index(string Name = "", string IsEnable = "0", string IsConfirm = "0", int pageIndex = 1, int pageSize = 10)
        {
            ...
            {
                result = proxy.GetEmailTempleteList(qc);
        ...
        // GET: EmailTemplete/Add
        [AuthCheck(Module = AdminModule.EmailTemplete, Right = SystemRight.Add)]
        ...
        // POST: EmailTemplete/Add
        [HttpPost]
        [ValidateInput(false)]
        [AuthCheck(Module = AdminModule.EmailTemplete, Right = SystemRight.Add)]
        public ActionResult Add(Msg_EmailTemplete model)
        {
            ...
            using (MessageClientProxy proxy = new MessageClientProxy(ProxyEx(Request)))
            {
                result = proxy.AddEmailTemplete(model);
                view = result.ToResultView();
            }
            return Json(view);
        }
        // GET: EmailTemplete/Detail/5
        [AuthCheck(Module = AdminModule.EmailTemplete, Right = SystemRight.Browse)]
        ...
        // GET: EmailTemplete/Edit/5
        [AuthCheck(Module = AdminModule.EmailTemplete, Right = SystemRight.Modify)]
        ...
        // POST: EmailTemplete/Edit/5
        [HttpPost]
        [ValidateInput(false)]
        [AuthCheck(Module = AdminModule.EmailTemplete, Right = SystemRight.Modify)]
        public ActionResult Edit(Msg_EmailTemplete model)
        {
            Result<int> result = new Result<int>();
            ResultView<int> view = new ResultView<int>();
            model.TemplateCon = model.TemplateCon.FilterHtml();
            ...
            using (MessageClientProxy proxy = new MessageClientProxy(ProxyEx(Request)))
            {
                result = proxy.UpdateEmailTemplete(model);
                view = result.ToResultView();
            }
            return Json(view);
        }
        //EditState
        [HttpPost]
        public ActionResult EditState(long emailTempleteId, int type)
        {
            Result<int> result = new Result<int>();
            using (MessageClientProxy proxy = new MessageClientProxy(ProxyEx(Request)))
            {
                result = proxy.AuditEmailTemplete(emailTempleteId, type);
            }
            return Json(result.ToResultView());
        }
        // GET: EmailTemplete/Delete/5
        [HttpPost]
        [AuthCheck(Module = AdminModule.EmailTemplete, Right = SystemRight.Delete)]
        public ActionResult Delete(string ids)
        {
            ...
            using (MessageClientProxy proxy = new MessageClientProxy(ProxyEx(Request)))
            {
                result = proxy.DeleteEmailTempleteByIds(list);
            }
            return Json(result.ToResultView());
        }
    }
