...
namespace hc.epm.Admin.Web.Controllers
{
    public class CheckItemController : BaseController
    {
        ...
        /// </summary>
        /// <returns></returns>
        [AuthCheck(Module = AdminModule.CheckItem, Right = SystemRight.Browse)]
        public ActionResult Index(string name = "", int pageIndex = 1, int pageSize = 10)
        {
            ...
            using (BusinessClientProxy proxy = new BusinessClientProxy(ProxyEx(Request)))
            {
                ...
                {
                    var cate = categoryList.Where(p => p.Id == item.ParentId).FirstOrDefault();
                    if (cate == null) continue;
                    item.ParentName = cate.ParentName;
                    item.Name = cate.Name;
                }
        ...
        /// </summary>
        /// <returns></returns>
        [AuthCheck(Module = AdminModule.CheckItem, Right = SystemRight.Add)]
        public ActionResult Add()
        {
            ViewBag.RectificationPeople = JsonConvert.SerializeObject(HelperExt.AsEnumList(typeof(RectificationPeople)));
            ViewBag.RectificationCompany = JsonConvert.SerializeObject(HelperExt.AsEnumList(typeof(RectificationCompany)));
            ViewBag.RoleType= typeof(RoleTypeEnum).AsSelectList(true);
            return View();
        }
        ...
        /// </summary>
        /// <returns></returns>
        [AuthCheck(Module = AdminModule.CheckItem, Right = SystemRight.Modify)]
        ...
        /// <returns></returns>
        [AuthCheck(Module = AdminModule.CheckItem, Right = SystemRight.Delete)]
        [HttpPost]
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult SelectParent(string roleType)
        {
        ...
        /// <returns></returns>
        [AuthCheck(Module = Common.AdminModule.CheckItem, Right = Common.SystemRight.Browse)]
        [HttpPost]
        public ActionResult GetCheckItemTree(string roleType,long pId = 0, bool last = true, string chk = "")
        {
            Result<List<Epm_CheckItem>> result = new Result<List<Epm_CheckItem>>();
            QueryCondition qc = new QueryCondition();
            qc.PageInfo.isAllowPage = false;
