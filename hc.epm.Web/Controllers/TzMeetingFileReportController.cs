...
namespace hc.epm.Web.Controllers
{
    ...
    /// 上会材料上报
    /// </summary>
    public class TzMeetingFileReportController : BaseWebController
    {
        // GET: TzMeetingFileReport
        [AuthCheck(Module = WebCategory.TzMeetingFileReport, Right = SystemRight.Browse)]
        public ActionResult Index(string projectName = "", string projectNature = "", string stationName = "", string companyId = "", string startTime = "", string projectState = "", string endTime = "", int pageIndex = 1, int pageSize = 10)
        {
            ...
            using (ClientSiteClientProxy proxy = new ClientSiteClientProxy(ProxyEx(Request)))
            {
                ...
                result = proxy.GetMeetingFileReportList(qc);
        ...
        /// <returns></returns>
        [AuthCheck(Module = WebCategory.TzMeetingFileReport, Right = SystemRight.Modify)]
        public ActionResult Add(long projectId)
        {
            using (ClientSiteClientProxy proxy = new ClientSiteClientProxy(ProxyEx(Request)))
            {
                //根据字典类型集合获取字典数据
                List<DictionaryType> subjectsList = new List<DictionaryType>() { DictionaryType.MeetingFileType };
                var subjects = proxy.GetTypeListByTypes(subjectsList).Data;

                var result = proxy.GetMeetingFileReportModel(projectId);

                //附件类型
                ViewBag.AnnexType = subjects[DictionaryType.MeetingFileType].ToList().ToSelectList("Name", "No", true);
        ...
        /// <returns></returns>
        [HttpPost]
        [AuthCheck(Module = WebCategory.TzMeetingFileReport, Right = SystemRight.Check)]
        public ActionResult UpdateState(string ids, string state)
        {
            ...
            using (ClientSiteClientProxy proxy = new ClientSiteClientProxy(ProxyEx(Request)))
            {
                result = proxy.UpdateMeetingFileReportState(idList, state);
            }
            return Json(result.ToResultView());
        }
