...
namespace hc.epm.Web.Controllers
{
    ...
    /// 项目批复信息
    /// </summary>
    public class TzProjectApprovalInfoController : BaseWebController
    {
        // GET: TzProjectApprovalInfo
        [AuthCheck(Module = WebCategory.TzProjectApprovalInfo, Right = SystemRight.Browse)]
        public ActionResult Index(string projectName = "", string projectNature = "", string stationName = "", string companyId = "", string startTime = "", string projectState = "", string endTime = "", int pageIndex = 1, int pageSize = 10)
        {
            ...
            using (ClientSiteClientProxy proxy = new ClientSiteClientProxy(ProxyEx(Request)))
            {
                ...
                result = proxy.GetTzProjectApprovalInfoList(qc);
        ...
        /// <returns></returns>
        [AuthCheck(Module = WebCategory.TzProjectApprovalInfo, Right = SystemRight.Modify)]
        public ActionResult Add(long projectId)
        {
            using (ClientSiteClientProxy proxy = new ClientSiteClientProxy(ProxyEx(Request)))
            {
                //根据字典类型集合获取字典数据
                List<DictionaryType> subjectsList = new List<DictionaryType>() { DictionaryType.NatureLand, DictionaryType.LandUse, DictionaryType.ApprovalInfoFileType, DictionaryType.LandPaymentType,DictionaryType.ProjectType };
                var subjects = proxy.GetTypeListByTypes(subjectsList).Data;
        ...
        /// <returns></returns>
        [HttpPost]
        [AuthCheck(Module = WebCategory.TzProjectApprovalInfo, Right = SystemRight.Check)]
        public ActionResult UpdateState(string ids, string state)
        {
            ...
            using (ClientSiteClientProxy proxy = new ClientSiteClientProxy(ProxyEx(Request)))
            {
                result = proxy.UpdateTzProjectApprovalInfoState(idList, state);
            }
            return Json(result.ToResultView());
        }
    }
