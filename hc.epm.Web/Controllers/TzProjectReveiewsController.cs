...
{
    /// <summary>
    /// 项目评审记录:TzProjectReveiews
    /// </summary>
    public class TzProjectReveiewsController : BaseWebController
    {
        [AuthCheck(Module = WebCategory.TzProjectReveiews, Right = SystemRight.Browse)]
        public ActionResult Index(string projectName = "", string projectNature = "", string stationName = "", string companyId = "", string startTime = "", string projectState = "", string endTime = "", int pageIndex = 1, int pageSize = 10)
        {
            ...
            using (ClientSiteClientProxy proxy = new ClientSiteClientProxy(ProxyEx(Request)))
            {
                ...
                result = proxy.GetTzProjectReveiewsList(qc);
        ...
        [AuthCheck(Module = WebCategory.TzProjectReveiews, Right = SystemRight.Modify)]
        public ActionResult Add(long projectId)
        {
            using (ClientSiteClientProxy proxy = new ClientSiteClientProxy(ProxyEx(Request)))
            {
                //根据字典类型集合获取字典数据
                List<DictionaryType> subjectsList = new List<DictionaryType>() { DictionaryType.OtherInfo,  DictionaryType.ReveiewsFileType,  DictionaryType.ConclusionCode };
                var subjects = proxy.GetTypeListByTypes(subjectsList).Data;
        ...
        /// <returns></returns>
        [HttpPost]
        [AuthCheck(Module = WebCategory.TzProjectReveiews, Right = SystemRight.Check)]
        public ActionResult UpdateState(string ids, string state)
        {
            ...
            using (ClientSiteClientProxy proxy = new ClientSiteClientProxy(ProxyEx(Request)))
            {
                result = proxy.UpdateTzProjectReveiewsState(idList, state);
            }
            return Json(result.ToResultView());
        }
    }
